...
(function () {
  ...
  ani.canvas = function (options) {
    ...
    canvas.on("object:scaling", function (e) {
      if (e.target) e.target.events("scaling", [e]);
    });
    canvas.on("object:rotating", function (e) {
      if (e.target ) e.target.events("rotating", [e]);
    });

    canvas.on("object:moving", function (e) {
      e.e = ani.fixMousePos(e.e);
      if (e.target) e.target.events("moving", [e]);
    });
  ...
  ani.editPages = function (schema) {
    ...
    var processItem = function (item, parent) {
      ...
      if (item.items) {
        ...
        if (item.input)
        {
          ...
          if (item.input == "text") {
            input$.append('<input type="text" class="card-black" />');
            ...
            input$.append('<div class="select-box select-box-2"><select></select></div>');
            ...
            input$[0].underlineValue = function (v) {
              this.colorPicker.smartEdit.val(v);
            };
          ...
          item$.addClass('button button-xxs button-dark');
          item$[0].onclick = item.onClick;
        }
        else if (item.title && !item.type) item$.html(item.title);
   ...
   div$[0].fields = {};
   div$.find(".page-input").each(function () {
     if (this.fieldName) {
  ...
  .editPages .range-slider > input{float:left; }.editPages .range-slider > input:last-child{opacity:0.3}
  .editPages .toggle .fac-checkbox > span { border: none !important;}
  .editPages .range-slider > input:first-child {width: 30%; position: absolute;top: -28px; right: 0;min-width: 40px;}
  .editPages .select-box,.editPages .select-box select  { margin-bottom: 0px; height: 40px;line-height: 40px;}
  .editPages .page-input {padding:5px;}
  .editPages .page-input input[type="text"] {border: solid 1px #777676  !important;text-align:center;height: 40px;line-height: 40px;font-size:110%;color:#c2c2c2;-webkit-box-shadow: -3px 5px 13px -6px rgba(0,0,0,0.4);-moz-box-shadow: -3px 5px 13px -6px rgba(0,0,0,0.4);box-shadow: -3px 5px 13px -6px rgba(0,0,0,0.4);}
  .editPages .page-input {padding:5px}
  .editPages .page-input em {font-size:110%;color:#d4d4d4;font-weight: 600;} 
  .editPages .toggle-classic i {right: 10px; color: #FFFFFF;} 
  .editPages .toggle-title {padding-left:10px;color: #FFFFFF;}  
  .editPages .toggle-active .toggle-classic .fa-chevron-down { color:  #FFFFFF;}
  .editPages .ios-slider { background: linear-gradient(to right, #c0c0c0 0%, #dbdbdb 100%);background-size: 100% 3px; background-position: center; background-repeat: no-repeat;}
  </style>*/
  }).toString().replace('/*', '').replace('*/', '');
